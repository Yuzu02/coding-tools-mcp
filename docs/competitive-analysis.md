...
| --- | --- | --- |
| Tool choice | One stable catalog of 20 low-level coding tools, including project-scoped `list_skills` / `read_skill`; no profiles or dynamic process tools | A fixed catalog reduces discovery and routing variance, but a host agent can still add its own tools |
