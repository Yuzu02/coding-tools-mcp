...
class ProjectContext:
    ...
    def server_instructions(self) -> str:
        sections = [
            ...
            (
                "Use apply_patch for deliberate source edits made by the agent. "
                "exec_command may modify files inside the configured workspace when running canonical workspace tools "
                "such as formatters, linters with autofix, code generators, package managers, migrations, or build tools; "
                "inspect the resulting diff and run the relevant checks afterward."
            ),
        ]
